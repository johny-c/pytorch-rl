...
class DQNAgent(Agent):
    ...
    def _get_q_update(self, experiences):  # compute temporal difference error for a batch
        ...
        state0_batch_vb = Variable(
            torch.from_numpy(np.array(tuple(experiences[i].state0 for i in range(len(experiences))))).type(self.dtype))
        action_batch_vb = Variable(
            torch.from_numpy(np.array(tuple(experiences[i].action for i in range(len(experiences))))).long())
        reward_batch_vb = Variable(
            torch.from_numpy(np.array(tuple(experiences[i].reward for i in range(len(experiences)))))).type(self.dtype)
        state1_batch_vb = Variable(
            torch.from_numpy(np.array(tuple(experiences[i].state1 for i in range(len(experiences))))).type(self.dtype))
        terminal1_batch_vb = Variable(torch.from_numpy(
            np.array(tuple(0. if experiences[i].terminal1 else 1. for i in range(len(experiences)))))).type(self.dtype)
    ...
    def _epsilon_greedy(self, q_values_ts):
        ...
        if self.training:  # linearly anneal epsilon
            self.eps = self.eps_end + max(0, (self.eps_start - self.eps_end) * (
                        self.eps_decay - max(0, self.step - self.learn_start)) / self.eps_decay)
        else:
    ...
    def _forward(self, observation):
        ...
        q_values_ts = self.model(
            Variable(state_ts, volatile=True)).data  # NOTE: only doing inference here, so volatile=True
    ...
    def fit_model(self):
        ...
        while self.step < self.steps:
            ...
            if self.step % self.prog_freq == 0:
                self.logger.warning("Reporting       @ Step: " + str(self.step) + " | Elapsed Time: " + str(
                ...
                self.logger.warning(
                    "Training Stats:   avg_reward:       {}".format(total_reward / nepisodes if nepisodes > 0 else 0.))
                ...
                self.logger.warning("Training Stats:   repisodes_solved: {}".format(
                    nepisodes_solved / nepisodes if nepisodes > 0 else 0.))
    ...
    def _eval_model(self):
        ...
        self.steps_std_log.append([self.step, np.std(np.asarray(eval_episode_steps_log))]);
        del eval_episode_steps_log
        self.reward_avg_log.append([self.step, np.mean(np.asarray(eval_episode_reward_log))])
        self.reward_std_log.append([self.step, np.std(np.asarray(eval_episode_reward_log))]);
        ...
        self.repisodes_solved_log.append(
            [self.step, (eval_nepisodes_solved / eval_nepisodes) if eval_nepisodes > 0 else 0])
        # plotting
        if self.visualize:
            self.win_v_avg = self.vis.scatter(X=np.array(self.v_avg_log), env=self.refs, win=self.win_v_avg,
                                              opts=dict(title="v_avg"))
            self.win_tderr_avg = self.vis.scatter(X=np.array(self.tderr_avg_log), env=self.refs,
                                                  win=self.win_tderr_avg, opts=dict(title="tderr_avg"))
            self.win_steps_avg = self.vis.scatter(X=np.array(self.steps_avg_log), env=self.refs,
            ...
            self.win_reward_avg = self.vis.scatter(X=np.array(self.reward_avg_log), env=self.refs,
            ...
            self.win_nepisodes = self.vis.scatter(X=np.array(self.nepisodes_log), env=self.refs,
                                                  win=self.win_nepisodes, opts=dict(title="nepisodes"))
            self.win_nepisodes_solved = self.vis.scatter(X=np.array(self.nepisodes_solved_log), env=self.refs,
                                                         win=self.win_nepisodes_solved,
                                                         opts=dict(title="nepisodes_solved"))
            self.win_repisodes_solved = self.vis.scatter(X=np.array(self.repisodes_solved_log), env=self.refs,
                                                         win=self.win_repisodes_solved,
    ...
    def test_model(self):
        ...
        self.steps_std_log.append([self.step, np.std(np.asarray(test_episode_steps_log))]);
        del test_episode_steps_log
        self.reward_avg_log.append([self.step, np.mean(np.asarray(test_episode_reward_log))])
        self.reward_std_log.append([self.step, np.std(np.asarray(test_episode_reward_log))]);
        ...
        self.repisodes_solved_log.append(
            [self.step, (test_nepisodes_solved / test_nepisodes) if test_nepisodes > 0 else 0.])
        # plotting
        if self.visualize:
            self.win_steps_avg = self.vis.scatter(X=np.array(self.steps_avg_log), env=self.refs,
                                                  win=self.win_steps_avg, opts=dict(title="steps_avg"))
